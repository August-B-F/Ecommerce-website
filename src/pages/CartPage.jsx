...
const Cart = () => {
  ...
  const totalPrice = cartItems.reduce((total, item) => total + item.price.formatted * item.quantity, 0).toFixed(2);
          ...
          <List>
            {cartItems.map((item) => (
              ...
              >
                ...
                />
                 <Typography variant="h6">${(item.price.formatted * item.quantity).toFixed(2)}</Typography>
              </ListItem>
            ))}
          </List>
